...
with st.sidebar:
    ...
    uploaded = st.file_uploader("Upload Obama Speeches CSV", type=['csv'])
...
if uploaded is None:
    ...
    <div style='text-align:center; padding: 5rem 2rem 3rem 2rem;'>
        ...
        <div style='margin-top: 2rem; font-size: 0.85rem; color: #555577;'>
            ← Upload your <code style="background:#1a1a2e; padding: 2px 6px; border-radius:4px; color: #e040fb;">obama_speeches.csv</code> in the sidebar to begin
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    features = [
        ...
        ("📦", "Text Mining", "BoW · TF-IDF · N-grams · Word2Vec"),
        ("💬", "Sentiment", "VADER + Aspect-Based Political Lexicon"),
        ("🗺️", "Topics", "LDA · LSA · K-Means · Hierarchical"),
        ("🏷️", "NER", "NLTK Rule-Based + spaCy ML-Based"),
        ("🤖", "Classification", "Naive Bayes · SVM · Logistic Regression"),
    ]
    for i, col in enumerate([col1, col2, col3]):
        for j in [i, i+3]:
            if j < len(features):
                ico, title, desc = features[j]
                col.markdown(f"""
                <div class='metric-card' style='margin-bottom:1rem;'>
                    <div style='font-size:1.8rem'>{ico}</div>
                    <div style='font-family: Playfair Display, serif; font-size: 1.1rem; color: #fff; margin: 0.3rem 0;'>{title}</div>
                    <div style='font-size:0.78rem; color:#666699;'>{desc}</div>
...
with st.spinner("🔄 Loading and preprocessing speeches..."):
    df_raw, df, vader = load_and_prepare(uploaded.read())
